// This file is responsible for getting the structure of all the AST nodes .
...
// The code below is example of LLVM RTTI . 
/*
The dynamic_cast<> C++ operator could be used for this.
The problem is that the required RTTI is only available if the C++ class has a virtual table attached
– that is, it uses virtual functions.But this makes the process bloated . 

To implement the LLVM-style RTTI, a public
enumeration containing a label for each subclass must be added. Also, a private member of this type
and a public getter are required. The private member is usually called Kind.
...
public:
  ...
  // this classof method is used to check if the instance is of requested type . 
